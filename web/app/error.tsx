  ...
  return (
    <div className={cn("flex min-h-[60vh] items-center justify-center")}> 
      ...
      >
        ...
        <h1 className={cn("mt-3 text-2xl font-semibold text-slate-900 dark:text-slate-100")}>
          页面遇到问题
        </h1>
        <p className={cn("mt-3 text-sm text-slate-600 dark:text-slate-300")}>
          我们已记录该问题，你可以尝试重新加载页面。
        </p>
        {error?.digest ? (
          <p className={cn("mt-2 text-xs text-slate-400")}>错误编号：{error.digest}</p>
        ) : null}
